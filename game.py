...
def next_level():
    global current_boss_level, boss_health, player_health, victory
    ...
    if current_boss_level < len(bosses):
        ...
        victory = True  # Trigger victory state
        current_boss_level = len(bosses)  # Prevent out-of-range errors



def render_victory_screen():
    """Display the victory message and background."""
    screen.blit(victory_bg, (0, 0))
    victory_text = story_font.render(
        "Thank you, Aria. You have restored Earth to its former vitality.", True, WHITE
    )
...
while running:
    ...
    if not game_started:  # Story screen logic
        draw_story()
        ...
        if victory:  # Handle victory state
            render_victory_screen()
            pygame.display.flip()
            clock.tick(FPS)
            continue  # Skip the rest of the loop iteration when victory is active
        ...
        if game_over:  # Handle game over state
            ...
            if keys[pygame.K_r]:
                ...
                player_health = player_max_health
                boss_health = bosses[current_boss_level]["health"]
                game_started = False  # Go back to the story screen
            pygame.display.flip()
            clock.tick(FPS)
            continue
        ...
        # Ensure every access of current_boss_level is within valid range
        if 0 <= current_boss_level < len(boss_sprites):
            screen.blit(boss_sprites[current_boss_level], (boss_x, boss_y))
        for bullet in bullets:
            screen.blit(bullet_img, (bullet[0], bullet[1]))

        if 0 <= current_boss_level < len(boss_attack_sprites):  # Safeguard for boss_attack_sprites
            for boss_bullet in boss_bullets:
                screen.blit(boss_attack_sprites[current_boss_level], (boss_bullet[0], boss_bullet[1]))


       # Draw health bars
        pygame.draw.rect(screen, RED, (50, HEIGHT - 50, 200, 20))  # Player health background
        pygame.draw.rect(screen, GREEN, (50, HEIGHT - 50, 200 * (player_health / player_max_health), 20))  # Player health bar

        # Check if current_boss_level is valid before drawing boss health
        if 0 <= current_boss_level < len(bosses):
            pygame.draw.rect(screen, RED, (WIDTH - 250, 50, 200, 20))  # Boss health background
            pygame.draw.rect(screen, GREEN, (WIDTH - 250, 50, 200 * (boss_health / bosses[current_boss_level]["health"]), 20))  # Boss health bar
